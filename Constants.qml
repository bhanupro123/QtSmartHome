        ...
        onClientConnected: {
            ...
            webSocket.onTextMessageReceived.connect(function (message) {
                ...
                if (message.startsWith("!") && message.endsWith("!")) {
                    ...
                    for (var i = 0; i < clientArray.length; i++) {
                        if (clientArray[i].id === client) {
                            clientArray[i].socket = webSocket
                            return
                        }
                    }
                    clientArray.push({
                ...
                } else if (message.startsWith(underGroundEsp)) {

                    if (message === kitchenAlive || message === motorTerminated
                            || message === tankAlive) {
                        motorStatus = message
                    ...
                    for (var i = 0; i < clientArray.length; i++) {
                        if (clientArray[i].socket) {
                            clientArray[i].socket.sendTextMessage(message)
                        }
                    }
                }
            })
            ...
            for (var i = 0; i < clientArray.length; i++) {
                if (clientArray[i].socket) {
                    clientArray[i].socket.sendTextMessage(
